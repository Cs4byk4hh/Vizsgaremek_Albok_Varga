...
const register = async (req, res) => {
    ...
    const letezofelhasznalo = await felhasznalok.findOne({
        where: {
    ...
    if (letezofelhasznalo) {
        if (letezofelhasznalo.email === email) {
            return res.status(400).json({message: 'Ezzel az emaillel már regisztrált felhasználó!'});
        }
        if (letezofelhasznalo.username = username) {
...
const login = async (req, res) => {
    try {
        ...
        if (!email || !username || !password) {
            return res.status(400).json({ message: 'Hiányzó adat! Kérlek add meg az emailt, a felhasználónevet és a jelszót is.' });
        }
        ...
        if (!letezofelhasznalo) {
            return res.status(404).json({ message: 'Hibás email cím vagy felhasználónév!' });
        }
